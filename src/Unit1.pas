 ...
 while not eof(f1) do
  begin
    repeat
      read(f1,c);
      s:=s+c;
      if (c in ['А'..'Я']) or (c in ['а'..'я'])then s1:=s1+c
      else c:=' ';
      if (eoln(f1)) and not(eof(f1)) then c:=' ';
    until (c=' ') or (eof(f1));


if s[1] in ['А'..'Я'] then k1:=1;
  for i:=1 to length(s) do
    if s[i] in ['А'..'Я'] then s[i]:=locase(s[i]);
for i:=1 to length(s1) do
  if s1[i] in ['А'..'Я'] then  s1[i]:=locase(s1[i]);
...
if (p<>1)and(length(s1)=2) then
  begin
    ...
    form2.sLabel1.Caption:='В слове "'+s1+'" ошибка';
    form2.sRadioButton8.Caption:='Оставить без изменений';
    form2.sRadioButton9.Caption:='Ввести свой вариант';
    form2.Show;
    form1.Enabled:=false;
...
begin
  ...
  begin
    ...
    if (ms3=ms1) and (ms2[length(ms3)+1]='П') then
      begin
        proverka:=1;
        break;
      end
...
for i:=1 to length(ms) do
 ...
 for i:=1 to length(ms1) do
    if ms1[i] in ['А'..'Я'] then ms1[i]:=locase(ms1[i]);
ms5:=ms1;
...
if (proverka()=1) and (sEdit1.Text<>'')  then
  begin
    ...
    for i:=1 to length(ms2) do
     if ms2[i]<>',' then mk:=mk+1 else break;   // Приставка
    form3.sLabel8.Caption:=copy(ms2,1,mk);
    delete(ms2,1,mk+1);
    mk:=0;

    for i:=1 to length(ms2) do
     if ms2[i]<>',' then mk:=mk+1 else break;      //Корень
    form3.sLabel7.Caption:=copy(ms2,1,mk);
    delete(ms2,1,mk+1);
    mk:=0;

    for i:=1 to length(ms2) do
     if ms2[i]<>',' then mk:=mk+1 else break;  //Суффикс
    form3.sLabel9.Caption:=copy(ms2,1,mk);
    delete(ms2,1,mk+1);
    mk:=0;

    for i:=1 to length(ms2) do
     if ms2[i]<>',' then mk:=mk+1 else break;      //Окончание
    form3.sLabel10.Caption:=copy(ms2,1,mk);
    delete(ms2,1,mk+1);
    mk:=0;

    for i:=1 to length(ms2) do
     if ms2[i]<>',' then mk:=mk+1 else break;    //Основа
